...
RSpec.describe "グループ用持ち物リスト管理", type: :system do
  ...
  describe "グループメンバーのアイテム追加（Turbo Stream）" do
    ...
    it "グループメンバーが新規アイテムを追加できること" do
      visit group_schedule_packing_list_path(group, schedule)
      ...
      # ページがリロードされていないことを確認
      expect(page).to have_current_path(group_schedule_packing_list_path(group, schedule))
    end
  ...
  describe "グループメンバーシップと権限" do
    ...
    it "グループメンバーではないユーザーはアクセスできないこと" do
      ...
      expect(page).not_to have_current_path(group_schedule_packing_list_path(group, schedule))
    end
  end
end
